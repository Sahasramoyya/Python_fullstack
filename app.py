...
def init_db():
    ...
    cursor=conn.cursor()
    cursor.execute(""" CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,email TEXT NOT NULL,password TEXT NOT NULL, dob TEXT NOT NULL,gender TEXT NOT NULL,course TEXT NOT NULL)""")
    conn.commit()
    conn.close()
...
def api_register():
    ...
    conn.commit()
    conn.close()
    return jsonify({"status":"success","message":"Regustration successful"})    
